    ...
    <div class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-md">
        <div class="flex justify-between items-start mb-4 gap-4">
            <div class="flex-1 min-w-0">
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white truncate">{{ $record->judul }}</h2>
                <div class="mt-2 flex items-center gap-2">
                    @php
                        // helper to compute readable text color
                        $computeTextColor = function ($hex) {
                            $hex = ltrim($hex ?? '', '#');
                            if (strlen($hex) === 3) {
                            ...
                            $text = $luminance > 0.65 ? '#111827' : '#ffffff';
                            return ['bg' => "#{$hex}", 'text' => $text];
                        };

                        $kategoriColor = $record->kategori?->color ?? '#F97316';
                        $kategoriColors = $computeTextColor($kategoriColor);
                        $subjekColor = $record->subjek?->color ?? null;
                        $subjekColors = $subjekColor ? $computeTextColor($subjekColor) : null;
                    @endphp

                    @if ($record->kategori)
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium" style="<?php echo 'background-color: ' . $kategoriColors['bg'] . '; color: ' . $kategoriColors['text'] . ';' ?>">{{ $record->kategori->nama }}</span>
                    @endif

                    @if ($record->subjek)
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium" style="<?php echo 'background-color: ' . $subjekColors['bg'] . '; color: ' . $subjekColors['text'] . ';' ?>">{{ $record->subjek->nama }}</span>
                    @endif
                </div>
            </div>
            @php
            ...
            @if ($filePath && Illuminate\Support\Facades\Storage::disk('public')->exists($filePath))
                <a href="{{ $fileUrl }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-primary-600 dark:bg-primary-500 text-white font-semibold rounded-lg hover:bg-primary-700 dark:hover:bg-primary-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                    <x-heroicon-o-arrow-down-tray class="w-5 h-5 mr-2" />
                    Unduh File
                </a>
            @endif
        </div>
        <p class="text-gray-500 dark:text-gray-300 mb-4">{{ $record->deskripsi }}</p>
        ...
        @if ($filePath && Illuminate\Support\Facades\Storage::disk('public')->exists($filePath))
            @if (in_array($extension, ['pdf', 'jpg', 'jpeg', 'png', 'gif']))
                <div class="border rounded-lg overflow-hidden mb-4 bg-white dark:bg-gray-900">
            ...
            @elseif (in_array($extension, $officeExtensions))
                <div class="border rounded-lg overflow-hidden mb-4 p-4 bg-white dark:bg-gray-900">
                    @if ($extension == 'txt')
                        <pre>{{ Illuminate\Support\Facades\Storage::disk('public')->get($filePath) }}</pre>
                    @else
                        @if ($fileSize > 5 * 1024 * 1024) {{-- 5MB --}}
                            <div class="p-4 bg-yellow-100 dark:bg-yellow-900 text-yellow-700 dark:text-yellow-200 rounded-lg text-center">
            ...
            @else
                <div class="p-4 bg-gray-100 dark:bg-gray-900 rounded-lg text-center">
                    <p class="text-lg font-semibold mb-2 text-gray-900 dark:text-white">Pratinjau tidak tersedia untuk tipe file ini.</p>
                    <p class="text-gray-600 dark:text-gray-300 mb-4">Anda dapat mengunduh file untuk melihatnya.</p>
                    <a href="{{ $fileUrl }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-primary-600 dark:bg-primary-500 text-white font-semibold rounded-lg hover:bg-primary-700 dark:hover:bg-primary-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                        <x-heroicon-o-arrow-down-tray class="w-5 h-5 mr-2" />
                        Unduh File
                    </a>
        ...
        <div class="mt-6">
            <a href="{{ \App\Filament\Resources\ArsipResource::getUrl('index') }}" class="inline-flex items-center px-4 py-2 bg-gray-300 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-semibold rounded-lg hover:bg-gray-400 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
